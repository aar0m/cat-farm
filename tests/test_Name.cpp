...
#include "../src/Utility/Name.cpp"  // NOLINT: This is Professor's inclusion of Name.cpp file
...
BOOST_AUTO_TEST_SUITE( test_Name )
   ...
   BOOST_AUTO_TEST_CASE( test_validate_name_with_special_chars ) {
      ...
      for( char const &character: "`~!@#$%^&*()_=+[]{}\\|;:'\",<.>/?" ) {
         string aString( 1, character );
         BOOST_CHECK_EQUAL( Name::validateNoSpecialChars( aString ), false );
      }
   }
   ...
   BOOST_AUTO_TEST_CASE( test_validate_name_does_not_start_with_number ) {
      ...
      for( char const &character: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" ) {
         if( character == '\0' ) { continue; }  // Range based for loops return the \0
         string aString( 1, character );
         BOOST_CHECK_EQUAL( Name::validateStartsWithAlpha( aString ), true );
      }

      for( char const &character: "`~!@#$%^&* ()_=+[]{}\\|;:'\",<.>/?1234567890-" ) {
         string aString( 1, character );
         aString += "Sam";
         BOOST_CHECK_EQUAL( Name::validateStartsWithAlpha( aString ), false );
      }
   }
