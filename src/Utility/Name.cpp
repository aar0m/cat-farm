...
#include "../config.h"  // For PROGRAM_NAME ANIMAL_FARM_RNG
#include "Name.h"       // Just cuz
#include "Trim.h"       // For trim() trim_edges()
...
std::string Name::getNextName() {
   if( nameType == FROM_FILE ) {
      ...
      ANIMAL_FARM_RNG randomDevice;

      int nameIndex = nameRNG( randomDevice );
...
///   - #FROM_SERIAL: Return `max(serial_t) - serial` (the number of serial
///                   numbers left, which should be a very large number)
...
/// newName should have had ::trim run on it before it's validated here.
///
/// The validation rules are:
///
///   - The name should not be empty.
///   - The name should be trimmed for whitespace (both left and right
...
bool Name::validateInteriorWhitespaceTrimmed( const string_view newName ) {
   ...
   string trialName { trim_edges( tempName.data() ) };  // Trim whitespace
